...
<head>
    ...
    <title>{{ __('store.sorry') }}@if ($error == 'store')
            {{ __('store.store_title') }}
        @else
            {{ __('store.page_title') }}
        @endif
    </title>
        ...
        <div class="container container--wide">
            <div class="e404-container">
                <div class="landing">
                    ...
                    <h3>{{ __('store.sorry') }}@if ($error == 'store')
                            {{ __('store.store_title') }}
                        @else
                            {{ __('store.page_title') }}
                        @endif
                ...
                <div class="msg">
                    <form class="feedback-error row" id="contact_form" method="post">
                        ...
                        <div class="col-md-12">
                            <textarea class="form-control" required name="content" placeholder="{{ __('site.details') }}"></textarea>
                        </div>
                        <div class="col-md-12">
                            <div class="controls">
                                <button id="btn-return" class="btn btn--circular btn--return">
                                    <svg enable-background="new 0 0 20 20" height="20px" version="1.1"
            ...
            $(document).on("submit", "form.feedback-error", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        output_html.html("<div class='text-success text-center'>" + result +
                            "</div><br>").fadeIn().delay(5000).fadeOut();
                    }
                });
            })
        });
